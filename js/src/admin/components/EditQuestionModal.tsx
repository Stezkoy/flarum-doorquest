import Form from 'flarum/common/components/Form';
import app from 'flarum/common/app';
import { IFormModalAttrs } from 'flarum/common/components/FormModal';
import FormModal from 'flarum/common/components/FormModal';
import Button from 'flarum/common/components/Button';
import ItemList from 'flarum/common/utils/ItemList';
import Stream from 'flarum/common/utils/Stream';
import Switch from 'flarum/common/components/Switch';
import Select from 'flarum/common/components/Select';
import Group from 'flarum/common/models/Group';

import type Mithril from 'mithril';
import type Question from '../../common/models/Question';
import type { SaveAttributes } from 'flarum/common/Model';

export interface IEditQuestionModalAttrs extends IFormModalAttrs {
  question: Question;
}

export default class EditQuestionModal<CustomAttrs extends IEditQuestionModalAttrs = IEditQuestionModalAttrs> extends FormModal<CustomAttrs> {
  protected questionText!: Stream<string>;
  protected answer!: Stream<string>;
  protected groupId!: Stream<number>;
  protected maxUses!: Stream<number>;
  protected activates!: Stream<boolean>;

  oninit(vnode: Mithril.Vnode<CustomAttrs, this>) {
    super.oninit(vnode);

    const question = this.attrs.question;

    this.questionText = Stream(question.question() || '');
    this.answer = Stream(question.answer() || '');
    this.groupId = Stream(question.groupId() || '');
    this.maxUses = Stream(question.maxUses());
    this.activates = Stream(question.activates() || false);
  }

  className() {
    return 'Modal--small EditQuestionModal';
  }

  title() {
    return app.translator.trans('stezkoy-doorquest.admin.modals.edit_question.title');
  }

  content() {
    return (
      <div className="Modal-body">
        <Form>{this.fields().toArray()}</Form>
      </div>
    );
  }

  fields(): ItemList<Mithril.Children> {
    const items = new ItemList<Mithril.Children>();

    items.add(
      'question',
      <div className="Form-group">
        <label>{app.translator.trans('stezkoy-doorquest.admin.modals.create_question.question.label')}</label>
        <textarea className="FormControl" bidi={this.questionText} disabled={this.loading} rows={3} />
      </div>,
      100
    );

    items.add(
      'answer',
      <div className="Form-group">
        <label>{app.translator.trans('stezkoy-doorquest.admin.modals.create_question.answer.label')}</label>
        <input className="FormControl" bidi={this.answer} disabled={this.loading} />
      </div>,
      90
    );

    items.add(
      'group',
      <div className="Form-group">
        <label>{app.translator.trans('stezkoy-doorquest.admin.modals.create_question.group.label')}</label>
        <Select
          name="groupId"
          options={this.getGroupsForInput()}
          value={String(this.groupId())}
          onchange={(val: string) => this.groupId(Number(val))}
          disabled={this.loading}
        />
      </div>,
      80
    );

    items.add(
      'maxUses',
      <div className="Form-group">
        <label>{app.translator.trans('stezkoy-doorquest.admin.modals.create_question.max_uses.label')}</label>
        <input className="FormControl" name="maxUses" type="number" bidi={this.maxUses} disabled={this.loading} />
      </div>,
      70
    );

    items.add(
      'activates',
      <div className="Form-group">
        <label>{app.translator.trans('stezkoy-doorquest.admin.modals.create_question.activates_user.label')}</label>
        <Switch name="activates" state={this.activates()} onchange={(checked: boolean) => this.activates(checked)} disabled={this.loading}>
          {'‎'}
        </Switch>
      </div>,
      60
    );

    items.add(
      'submit',
      <div className="Form-group">
        <Button className="Button Button--primary" type="submit" loading={this.loading}>
          {app.translator.trans('stezkoy-doorquest.admin.modals.edit_question.submit_button')}
        </Button>
      </div>,
      -10
    );

    return items;
  }

  getGroupsForInput() {
    let options: { [key: string]: string } = {};

    app.store.all('groups').map((group) => {
      const groupCasted = group as Group;
      if (groupCasted.id() === Group.GUEST_ID) {
        return;
      }
      options[groupCasted.id() as string] = groupCasted.nameSingular();
    });

    return options;
  }

  data(): SaveAttributes {
    return {
      question: this.questionText().trim(),
      answer: this.answer().trim(),
      groupId: Number(this.groupId()),
      maxUses: Number(this.maxUses()),
      activates: this.activates(),
    };
  }

  onsubmit(e: SubmitEvent) {
    e.preventDefault();

    this.loading = true;

    this.attrs.question
      .save(this.data(), { errorHandler: this.onerror.bind(this) })
      .then(this.hide.bind(this))
      .catch(() => {
        this.loading = false;
      });
  }
}
